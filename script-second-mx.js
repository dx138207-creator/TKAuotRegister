...
const { runAppiumSecondScriptMx } = require("./appium-script-mx");
...
async function main() {
  ...
  try {
    ...
    console.log("[开始] 执行墨西哥二次注册主流程（从Continue with Google开始）");
    await runAppiumSecondScriptMx({
      host: APPIUM_HOST,
      port: APPIUM_PORT,
      udid: device.udid,
      email: device.email,
...
main().catch((err) => {
  console.error("墨西哥二次注册脚本执行失败:", err);
  process.exit(1);
});
