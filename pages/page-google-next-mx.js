async function clickGoogleSiguiente(driver, stage = "当前页") {
  const nativeSelectors = [
    ...
    '//*[@text="SIGUIENTE" or @content-desc="SIGUIENTE"]'
  ];
  ...
  async function tryClickByNativeSelectors() {
    for (const selector of nativeSelectors) {
      ...
      for (const element of elements) {
        ...
        await element.click();
        console.log(`已点击 SIGUIENTE 按钮（${stage}）(native: ${selector})`);
        return true;
      }
    }
    return false;
    ...
    try {
      ...
      for (const selector of webviewSelectors) {
        ...
        for (const element of elements) {
          ...
          await element.click();
          console.log(`已点击 SIGUIENTE 按钮（${stage}）(webview: ${selector})`);
          return true;
        }
      }
      return false;
  ...
  while (Date.now() < endTime) {
    ...
    console.log(`等待 SIGUIENTE 按钮出现（${stage}）...`);
    await driver.pause(pollIntervalMs);
  }

  throw new Error(
    [
      "未找到 Google 的 SIGUIENTE 按钮。",
...
module.exports = { clickGoogleSiguiente };
